...
public class DesertdunesBiome extends NewmountainsModElements.ModElement {
	public static Biome biome;
	public DesertdunesBiome(NewmountainsModElements instance) {
		super(instance, 3);
	...
	private static class BiomeRegisterHandler {
		...
		public void registerBiomes(RegistryEvent.Register<Biome> event) {
			if (biome == null) {
				...
				biome = new Biome.Builder().precipitation(Biome.RainType.NONE).category(Biome.Category.DESERT).depth(1.5f).scale(0.7000000000000001f)
						.temperature(2f).downfall(0f).setEffects(effects).withMobSpawnSettings(mobSpawnInfo.copy())
						.withGenerationSettings(biomeGenerationSettings.build()).build();
				event.getRegistry().register(biome.setRegistryName("newmountains:desert_dunes"));
			}
		}
	}
	@Override
